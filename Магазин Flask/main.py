...
def index():
    ...
    return render_template("index.html", news=good, show='goods')

@app.route("/show_news")
def show_news():
    db_sess = db_session.create_session()
    news = db_sess.query(News).filter(News.is_private != True)
    return render_template("index.html", news=news, show='news')
...
def edit_goods(id):
    ...
    if request.method == "GET":
        ...
        if goods:
            ...
            # imname = goods.image
            # form.price.image = goods.image
            form.is_private.data = goods.is_private
        else:
            abort(404)
    if form.validate_on_submit():
        db_sess = db_session.create_session()
    ...
    return render_template('goods.html',
                           title='Редактирование сведений о товаре',
                           form=form
                           )
